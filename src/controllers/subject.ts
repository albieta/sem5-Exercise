...
import { insertSubject,getSubject,getSubjects,updateSubject,deleteSubject, matriculateSubject, getSubjectsByUser, getUsersBySubject } from "../services/subject";
...
const get_SubjectsByUser = async ({params}:Request,res:Response) => {
    try{
        const {idUser} = params;
        const response = await getSubjectsByUser(idUser);
        const data = response ? response: "NOT_FOUND";
        res.send(response);
    } catch(e) {
...
const get_UsersBySubject = async ({params}:Request, res:Response) => {
    try{
        const {idSubject} = params;
...
export{get_Subject,get_Subjects,post_Subject,update_Subject,delete_Subject,matriculate_Subject, get_SubjectsByUser, get_UsersBySubject};